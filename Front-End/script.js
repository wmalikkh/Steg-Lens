    ...
    if (button && activeInput) {
      button.onclick = () => {
        Swal.fire({
          ...
          html: `We accept only PNG, JPEG, or GIF images under 10MB.<br>Please ensure your image meets these requirements.`,
          confirmButtonText: 'OK',
  ...
  if (activeInput) {
    activeInput.addEventListener("change", async function () {
      ...
      if (unsupportedTypes.some(t => file.type.startsWith(t))) {
        return showPopup("❌ Unsupported file type. Please upload an image (PNG, JPEG, GIF).", true);
      }

      const maxSize = 10 * 1024 * 1024;
      const allowedTypes = ["image/png", "image/jpeg", "image/gif"];
      const needsFix = file.size > maxSize || !allowedTypes.includes(file.type);
      ...
      if (needsFix) {
        const confirmFix = await Swal.fire({
          ...
          html: 'This image is not supported or exceeds 10MB.<br>Do you want to convert and resize it automatically?',
        ...
        try {
          ...
          this.files = dt.files;
          file = fixedFile;
          showPopup("⚠️ Image converted to PNG and resized due to size/format.");
        } catch (e) {
          console.error("Auto-fix failed", e);
          stopVerifyBar();
          return showPopup("❌ Failed to fix image", true);
        }
      }
